...
public class CSVImportExamplePostprocessor : AssetPostprocessor
{
    static void OnPostprocessAllAssets(string[] importedAssets, string[] deletedAssets, string[] movedAssets, string[] movedFromAssetPaths)
    {
        foreach (string str in importedAssets)
        {
			...
			if (str.IndexOf("_HCSV.csv") != -1)
			{
				TextAsset data = AssetDatabase.LoadAssetAtPath<TextAsset>(str);
				string assetfile = str.Replace(".csv", ".asset");
				HoudiniData gm = AssetDatabase.LoadAssetAtPath<HoudiniData>(assetfile);
				if (gm == null)
				{
					gm = new HoudiniData();
					AssetDatabase.CreateAsset(gm, assetfile);
				...
				gm._entries = CSVSerializer.Deserialize<HoudiniData.Entry>(data.text);
